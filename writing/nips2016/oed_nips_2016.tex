...
\marginnote{Different experiments are optimal depending on the prior on models.
  Notice if $(P(m_{\text{fair}}), P(m_{\text{bias}})) = (5/8, 3/8)$, an \texttt{H} response is not
informative. Consider
\begin{align*}
  P(m|\texttt{H}) &= \frac{P(m)P(\texttt{H}|m)}{P(\texttt{H})} \\
  &\propto P(m)P(\texttt{H}|m) \\
  P(m_{\text{fair}} | \texttt{H}) &\propto (5/8)(1/2) = 5/16 \\
  P(m_{\text{bias}} | \texttt{H}) &\propto (3/8)(5/6) = 5/16
\end{align*}
}
...
Under $m_{\text{fair}}$, $p(\texttt{H}) = \frac{1}{2}$ but under $m_{\text{bias}}$, $p(\texttt{H})
= \frac{5}{6}$.
\hl{In this case, \emph{either} experimental response would be informative.}
...
\marginnote{
  ...
  How ``different'' the probabilities expressed by the models are, averaged over the common support of the two models $P$ and $Q$
  \begin{align*}
...
\hl{If, however, we think $M$ may not contain the true model, then an uninformative prior $p(y ; x)
\propto 1$ may be more appropriate.}\marginnote{i.e.\ uniform}
...
var g = function(){
  ...
  condition(x >= 2) (@ \hl{// i.e. factor(x >= 2 ? 0 : -Infinity)} @)
	return x
}
Infer(g) // returns the table p(2) = 6/11, p(3) = 4/11, p(4) = 1/11
(@ \hl{// i.e. Infer(\{method: `enumerate'\}, g)} @)
\end{lstlisting}
...
This provides a useful separation---\hl{we distinguish \emph{what} we wish to compute from
\emph{how} we try to compute it.}\marginnote{\emph{what} to compute: the model specified
probabilistically. \emph{how} to compute it: various inference algorithms}
...
Also, \hl{writing OED as a probabilistic program gives us access to algorithms that are more
  sophisticated than previous research has considered (e.g., mixtures of enumeration and HMC for
experiment spaces that have continuous and discrete subspaces).}
Finally, note that we implement search for the optimal experiment using inference.
This is not essential---we could also replace the outermost \lstinline|Infer()| call with an
optimization procedure (e.g., \lstinline|Search()|).\marginnote{lots of CPS magic happening here}
  ...
  Infer(function() {                      // search over x
    ...
    var KLDistrib = Infer(function() {    // compute KL for each y
      var y = ySample()                   // ${\color{purple} p(y ; x)}$
                                          (@ \hl{// Assume responses are universal across experiments} @)
      var mPosterior = Infer(function() { // $\color{purple} P(M \mid Y = y)$
        var m = mSample() (@ \hl{// Infer will search through all the possible model samples} @)
        factor(score(m(x), y)) (@ \hl{// mPosterior's dist is defined by conditional probabilities of m} @)
        return m
      })
...
For now, we assume that the experiment will include data from a single participant, so the
experiment space $X$ is the Cartesian product $\{1\} \times \{\texttt{H}, \texttt{T}\}^4$
representing the fixed sample size of 1 and sequence space.\footnote{Our notion of ``experiment''
is quite general, including traditional components like stimulus properties (e.g., coin sequence)
as well as other components like dependent measure and sample size.}\marginnote{If e.g. 2
  participants, we have experiments (1, \texttt{HHHH}), (2, \texttt{HHHH}), (1, \texttt{HHHT}), (2,
  \texttt{HHHT}), \ldots}
...
(@ \hl{// fairCoin totally ignores the sequence input arguments} @)
(@ \hl{// fairCoin($\cdot$) = \{T: 0.5, F: 0.5\}} @)
...
\marginnote{Notice our models are that each of the $n$ participants follow the same model. Can the
model space account for variability by enumerating the possible combinations? (e.g. 5 bias, 10
markov, 5 fair?) Would it make any sense?}
\begin{figure}[t]
...
\hl{This makes sense---the bias model learns a 0.5 probability of heads and so assigns equal
  probability of heads and tails to the next flip, whereas the Markov model learns that the
  transition probability is high and assigns high probability to the opposite of whatever outcome
was observed last} (\lstinline{T} for \lstinline{THTH} and \lstinline{H} for \lstinline{HTHT}).
...
In the full bair--bias--Markov comparison (Fig.~\ref{fig:run-coin}b, right), the worst experiments
(e.g., \lstinline{TTHH}) are again cases where all models make similar predictions.\marginnote{How
is expected information gain computed in the case of 2+ models?}
...
\emph{This function is non-linear and, crucially, the rank ordering of experiments can
change.}\marginnote{TODO: Do the math---why?}
...
Our tool eliminates the need to manually comb large spaces for good experiments; we hope this will
free scientists and engineers to work on the more interesting problems---devising empirical
paradigms and building models.\marginnote{At a certain point, space becomes too large even for the
computer}
...
It could be useful to integrate our system into multi-objective optimization systems for balancing
multiple design considerations (e.g., informativeness, cost, ethics).\marginnote{Right now can do this post-hoc}
