...
var g = function(){
	...
	condition(x >= 2) // i.e. factor(x >= 2 ? 0 : -Infinity)
	return x
}
  ...
  Infer(function() {                      // search over x
    ...
    var KLDistrib = Infer(function() {    // compute KL for each y
      var y = ySample()                   // ${\color{purple} p(y ; x)}$
                                          // Assume responses are universal across experiments
      var mPosterior = Infer(function() { // $\color{purple} P(M \mid Y = y)$
        var m = mSample() // Infer will search through all the possible model samples
        factor(score(m(x), y)) // mPosterior's dist is defined by conditional probabilities of m
        return m
      })
